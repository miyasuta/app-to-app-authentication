...
class IdentityService extends Service {
    ...
    async getJwks(token, contextConfig) {
        ...
        const cachedResponse = await this.jwksCache.getOrRequest(cacheKey, buildJwksRequest, { correlationId: contextConfig.correlationId });
        if (cachedResponse instanceof ResponseError && cachedResponse.responseCode === 400) {
            /**
             * Online validation in JWKS endpoint may have negative result for the provided jwksParams.
             * In this case, the response will have status 400 and it should result in a WrongAudienceError instead of a (subclass of) NetworkError.
             */
            throw new WrongAudienceError(token, this, `The online validation in the JWKS endpoint of the Identity Service instance responded with 400: '${cachedResponse.responseText}'. This indicates that the token should not be accepted by this service.`);
        } else if (cachedResponse instanceof Error) {
    ...
    async getTokenUrl(grant_type, { correlationId } = {}) {
        const openidConfiguration = await this.getOpenIDConfiguration({ correlationId });
     ...
     * @param {Token} token token from which issuer is extracted
