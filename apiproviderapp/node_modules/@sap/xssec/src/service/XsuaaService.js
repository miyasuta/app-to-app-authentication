...
const { HTTPS_SCHEME, ZID_QUERY_PARAMETER } = require('../util/constants');
...
class XsuaaService extends Service {
    ...
    async getJwks(token, contextConfig) {
        ...
        if (cachedResponse instanceof Jwks) {
            return cachedResponse;
        } else if (cachedResponse instanceof ResponseError && cachedResponse.responseCode === 400) {
            /**
             ...
             */
            throw new WrongAudienceError(token, this, `The online validation in the JWKS endpoint of the XSUAA server responded with 400: '${cachedResponse.responseText}'. This indicates that the token should not be accepted by this service.`);
     ...
     */
    async getTokenUrl(grant_type, { tenant } = {}) {
        let baseUrl;
        if (tenant) {
