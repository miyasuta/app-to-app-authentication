...
async function jsonRequest(url, request, { requestName = "<requestName>", correlationId, extractHeaders } = {}) {
    ...
    try {
        ...
        const error = new RequestError(url, request);
    ...
    if(!response.ok) {
        ...
        const error = new ResponseError(url, request, response.status, responseText);
        LOG.error(error.message, {correlationId});
        throw error;
    }
